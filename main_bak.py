...
class Pet:
        ...
        if self.stage == STAGE_EGG and self.stage_timer > self.hatch_time:
            ...
            if (self._dark_pct() > DARK_THRESHOLD
                ...
                if all(s > 85 for s in stats):
                    ...
                    self.premium_timer = max(0.0, self.premium_timer - dt * 2)
    ...
    def feed(self):
        ...
        self.weight = min(50, self.weight + 3.0)
    ...
    def play(self):
        ...
        self.weight = max(5, self.weight - 1.0)
...
class Game:
    ...
    def _play_evolution(self):
        if self.evolution_sound is not None:
            try:
        ...
        if btn == 0:
            ...
            if len(self.name_input) > 0:
                ...
                self.name_input = random.choice(
                    ...
                    "GLITCH", "LUNA", "NOVA", "STAR", "COMET", "ORBIT", "RAT", "BLOOM", "PUSSY",
    ...
    def _update_wake_chance(self, dt):
        if not hasattr(self, "_wake_check_t"):
            self._wake_check_t = 0.0
        if not hasattr(self, "_was_full_asleep"):
            self._was_full_asleep = False
        p = self.pet
        full_asleep = bool(
            p is not None
            and getattr(p, "sleeping", False)
            and p.state not in (STATE_DEAD, STATE_GHOST)
            and p.energy >= 99.5)
        if full_asleep and not self._was_full_asleep:
            self._wake_check_t = 0.0
        self._was_full_asleep = full_asleep
        if not full_asleep:
            self._wake_check_t = 0.0
            return
        self._wake_check_t -= dt
        if self._wake_check_t <= 0.0:
            self._wake_check_t = 60.0
            if random.random() < 0.30:
                self._wake_up_pet()
                print("[wake] Energie voll - Pet ist von allein aufgewacht")
    ...
    def _notify(self, title, msg):
        if not getattr(self, "notifications_on", True):    # <<< GATE 1: Sofort-Sendungen
            return False
        try:
            if platform() == "android":
    ...
    def _toggle_notifications(self):
        ...
        if self.notifications_on:
            ...
            self._cancel_pet_alarm()     # scharfe Alarms sofort toeten

    def _schedule_pet_warning(self, secs, reason):
        warn_in = max(DEATH_WARN_MIN, int(secs) - DEATH_WARN_LEAD)
        msg = self._need_message(reason)
        title = self._notif_title()              # einmal ziehen, für Alarm UND Fallback
    ...
    def _schedule_evolve_warning(self):
        ...
        if not secs:
            ...
            return
        if secs <= EVO_NOTIF_LEAD: #+ EVO_ALARM_MIN:
            #return                      # Vorlauf-Fenster vorbei -> NICHT nachfeuern
            msg = self._need_message(reason)
            title = ("PseudoPet schlüpft!" if reason == "hatch"
                     else "PseudoPet entwickelt sich!")
            self._notify(title, msg)
            self._last_notif_time = time.time()
            return
        warn_in = max(EVO_ALARM_MIN, int(secs) - EVO_NOTIF_LEAD)
        msg = self._need_message(reason)
        title = "PseudoPet schlüpft!" if reason == "hatch" else "PseudoPet entwickelt sich!"
    ...
    def _predict_poop_time(self):
        ...
        if p.stage == STAGE_EGG or p.state == STATE_HATCHING:
            return None
        to_next = max(0.0, POOP_TIMER_GATE - p.poop_timer) + POOP_WAIT_EXPECTED
        missing = POOP_WARN - p.poops
        return to_next + (POOP_TIMER_GATE + POOP_WAIT_EXPECTED) * (missing - 1)
    ...
    def _send_test_notification(self):
        self._notify("PseudoPet", "Test erfolgreich!")
    ...
    def run(self):
        ...
        while running:
            ...
            if c_held:
                ...
                self._quit_hold_t = 0.0
            if dt > 5.0 and frame > 2:
                if self.pet and self.pet.state not in (STATE_DEAD, STATE_GHOST):
                    self.offline_report = self.pet.simulate_offline(dt)
                    self.mode = "offline"
                    self.save()                                   # <<< Härtung
                    print(f"[offline] {dt / 60:.0f} min simuliert (dt-Weg)")   # <<< zum Testen
                dt = 0.1
            self.fly_t += dt
